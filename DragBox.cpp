...
bool DragBox::pick(const DragBox::Point& start,const DragBox::Vector& direction,double* maxLambda)
	{
	/* Initialize intersection result: */
	double lambdaMin=maxLambda!=0?*maxLambda:Math::Constants<double>::max;
	currentDragMode=NONE;
			...
			if(denominator!=0.0)
				...
				if(determinant>=0.0)
					...
					if(lambda>=0.0)
						...
						double angleCos=(normal1*normal2)/Math::sqr(rotateCylinderRadius);
						double angle;
						if(angleCos>=1.0)
							angle=0.0;
						else if(angleCos<=-1.0)
							angle=Math::Constants<double>::pi;
						else
...
void DragBox::draw(void) const
	...
	GLColor<GLfloat,4> baseColor=glGetCurrentColor<GLfloat>();
	GLColor<GLfloat,4> highlightColor(1.0f,1.0f,0.0f);
