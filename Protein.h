...
class Protein
	...
	class ChainAtom:public Atom // Class for atoms inside proteins
		...
		int getBackboneDistance(void) const; // Calculates distance (in bonds) from atom to nearest backbone atom
		};
	
	struct DihedralAnglePair // Structure representing the dihedral angles of an amino acid residue
		{
		/* Elements: */
		public:
		Scalar phi,psi; // Phi and psi angles
		
		/* Constructors and destructors: */
		DihedralAnglePair(void) // Dummy constructor
			{
			}
		DihedralAnglePair(Scalar sPhi,Scalar sPsi) // Elementwise constructor
			:phi(sPhi),psi(sPsi)
			{
			}
		
		/* Methods: */
		DihedralAnglePair& delta(const DihedralAnglePair& other) // Calculates a delta from the other angle pair to this angle pair
			{
			phi=other.phi-phi;
			psi=other.psi-psi;
			return *this;
			}
		DihedralAnglePair& negate(void) // Negates a dihedral angle delta
			{
			phi=-phi;
			psi=-psi;
			return *this;
	...
	class Residue // Class for amino acid residues
		...
		friend class StructureSelector;
		friend class BondRotator;
		friend class ProteinManipulator;
		friend class ProteinRenderer;
		...
		enum AminoAcid // Enumerated type for standard amino acids (PDB abbreviations)
			{
			ALA=0,ARG,ASN,ASP,ASX,CYS,GLN,GLU,GLX,GLY,
		...
		/* Constructors and destructors: */
		Residue(const char* sResidueName,int sResidueIndex,SecondaryStructure* sSecondaryStructure);
		...
		MD::Point getSideChainPosition(void) const; // Returns single point representing entire side chain
		DihedralAnglePair calcDihedralAngles(void) const; // Calculates the residue's dihedral angles
		};
	...
	class SecondaryStructure // Class to separate proteins into secondary structures
		...
		unsigned int version; // Version number of the secondary structure to track protein changes
		
		/* Constructors and destructors: */
		SecondaryStructure(StructureType sStructureType) // Starts building a secondary structure
			:structureType(sStructureType),
			 residueBegin(0),residueEnd(0),
			 pred(0),succ(0),
			 version(1)
			{
			};
		...
		unsigned int getVersion(void) const // Returns version number of secondary structure
			{
			return version;
			};
		};
	...
	class ResidueCreator // Class to create residues (and proteins) one atom at a time
		...
		bool buildingBackbone; // True if the residue's backbone is being constructed
		...
		void newResidue(const char* newAbbreviatedName,int newResidueIndex); // Starts adding a new residue
	...
	class StructureSelector // Class to encapsulate selection/modification of secondary structures
		...
		void invalidate(void); // Invalidates the selector
		friend bool operator==(const StructureSelector& ss1,const StructureSelector& ss2) // Equality operator
			{
			return ss1.structure==ss2.structure;
			};
		friend bool operator!=(const StructureSelector& ss1,const StructureSelector& ss2) // Inequality operator
			{
			return ss1.structure!=ss2.structure;
			};
		...
		void getDihedralAngles(DihedralAnglePair angles[]) const // Returns dihedral angles of all residues in structure
			{
			protein->getDihedralAngles(firstResidueIndex,numResidues,angles);
			};
		void changeDihedralAngles(const DihedralAnglePair deltaAngles[],int direction) const // Changes dihedral angles of all residues in structure
			{
			protein->changeDihedralAngles(firstResidueIndex,numResidues,deltaAngles,direction);
			};
		void setDihedralAngles(const DihedralAnglePair angles[],int direction) const // Sets dihedral angles of all residues in structure
			{
			protein->setDihedralAngles(firstResidueIndex,numResidues,angles,direction);
			};
		};
	...
	unsigned int secondaryStructureVersion; // Version number to keep track of changes of secondary structure list
	...
	/* Methods: */
	static Scalar getBondRadius(void) // Returns the bond radius
		{
		return bondRadius;
		}
	...
	unsigned int getSecondaryStructureVersion(void) const // Returns version number of secondary structure list
		{
		return secondaryStructureVersion;
		};
	...
	ConstAtomIterator pickAtom(const Point& p) const; // Selects an atom by specifying a point
	...
	Residue* pickResidue(const Point& p); // Selects residue by specifying a point
	...
	Point getResidueSideChainCentroid(const Residue* rPtr) const; // Returns centroid of a residue's side chain
	...
	void getDihedralAngles(int startResidue,int numResidues,DihedralAnglePair angles[]) const; // Ditto
	void changeDihedralAngles(int startResidue,int numResidues,const DihedralAnglePair deltaAngles[],int direction,bool stopAtLastResidue =false); // Applies dihedral angle increments to multiple residues; stops transforming atoms at last affected residue if flag is true
	void setDihedralAngles(int startResidue,int numResidues,const DihedralAnglePair angles[],int direction); // Sets dihedral angles of multiple residues
